    ...
    if not request.user.is_authenticated:
        ...
        if request.user.id == 1:
            if request.method == 'POST':
                ...
                if form.is_valid():
                    neighborhood = Neighborhood(neighborhood_name=request.POST['neighborhood_name'])
                    neighborhood.save()
            ...
            if user is None:
                ...
                user.save()
            if user.neighborhood_name is None:
                title = 'Neighborhood'
                neighborhoods = Neighborhood.objects.all()
                return render(request,'index.html',{'neighborhoods':neighborhoods})
            else:
                user = UserProfile.objects.filter(user = request.user).first()
                return redirect(reverse('neighborhood',args=[user.neighborhood_name.id]))
...
def neighborhood(request,neighborhood_id):
    if request.user.id == 1:
        ...
        return render(request,'neighborhood.html',{'neighborhood':neighborhood,'members':members})
    else:
        neighborhood = Neighborhood.objects.get(id = neighborhood_id)
        user = UserProfile.objects.filter(user = request.user).first()
        ...
        return render(request,'neighborhood.html',{'posts':posts,'form':form,'user':user,'businesses':businesses,'neighborhood':neighborhood,})
